...
- 이곳에 사용된 모든 이미지들은 우리가 복원해본 과정에서 나타난 이미지임.
...
### 4. 결과
- 한 장의 파노라마 이미지만을 사용해서 방을 3D로 복원할 수 있음.
- SFM으로 진행한 3D Reconstruction과 비교 시, 복원까지 적은 시간이 소요되었고 유리 또한 문제없이 복원됨.

### 5. 문제점
- 직사각형이 아닌 방의 경우 내부 구조가 제대로 구현되지 않음. 모든 벽면이 수직한다는 Manhattan World 가정의 근본적인 문제로 한 점에 대해 세 모서리가 직교하지 않는 공간에 대해서도 고려해야 함.
- 테스트할 데이터셋이 부족함.<br><br>

### 6. 추후 계획
- 직사각형이 아닌 방의 경우에도 내부 구조를 정확히 구현하도록 개선할 것.
- 더 좋은 성능의 GPU를 사용하여 더 많이 데이터셋을 가지고 모델을 돌려볼 것.


### 7. HorizonNet 훈련 및 테스트 과정
